...
import { MetricCard, Recommendation, PerformanceStatus } from '@/types/analysis-architecture';
...
export function analyzeLlmsTxt(llmsTxtData: any): MetricCard {
  ...
  // Determine status independently of score to avoid 0/0 = NaN
  const status: PerformanceStatus = llmsTxtData?.success === true ? 'excellent' : 'error';
